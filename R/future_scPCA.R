#' Paralellized Sparse Constrastive Principal Component Analysis
#'
...
#'   the method described in \insertRef{Zou2006}{scPCA}. Paralellization is
#'   implemented using the \code{future.apply} package.
#'
...
#'   n_centers = 4
#' )
future_scPCA <- function(target, background, center = TRUE, scale = FALSE,
                  n_eigen = 2,
  ...
  c_contrasts <- future_contrastiveCov(target, background, contrasts,
                                       center, scale)
  ...
  opt_params <- future_fitGrid(target, center, scale, c_contrasts, contrasts,
                               penalties, n_eigen,
