---
title: "Sparse contrastive principal components analysis with `scPCA`"
...
A similar simulation scheme is provided in \cite{abid2018exploring}.
...
demonstrate use of the `SingleCellExperiment` container in the [vignette from
that package](https://www.bioconductor.org/packages/release/bioc/vignettes/SingleCellExperiment/inst/doc/intro.html#2_creating_singlecellexperiment_instances).
...
```{r sce_setup, message=FALSE, eval=FALSE}
library(scRNAseq)
...
a small subset of the data, limited to the 100 genes with the highest variance,
after log-transforming the raw counts. Our goal here is to demonstrate a typical
assessment of single-cell RNA-seq data (and data from similar assays) using the
tools made available in the `scPCA` package, a standard analysis would follow a
workflow largely similar to the above, though without such convenient reductions
in the complexity of the available data.

```{r make_sce_sub, message=FALSE, eval=FALSE}
...
In order to use either contrastive PCA or its sparse variant, it's necessary to
have a background data set. Absent such a version of the `allen` data, we use
the `r n_genes` genes with the lowest variance as the background:

```{r make_sce_background, message=FALSE, eval=FALSE}
...
generate PCA, cPCA, and scPCA representations of the reduced `allen` data,
storing these in `SingleCellExperiment` object using the `reducedDims` method.

Below, we perform standard PCA on the experimental subset from the `allen` data
and perform both cPCA and scPCA using the `scPCA` function, storing each in a
separate object. After applying each of these dimension reduction techniques,
we store the resultant objects in a `SimpleList` that is then appended to the
`SingleCellExperiment` object using the `reducedDims` accessor.

```{r perform_dimred, message=FALSE, eval=FALSE}
...
# first, PCA
pca_out <- prcomp(t(log1p(assay(sce_sub))))

# next, cPCA
cpca_out <- scPCA(t(log1p(assay(sce_sub))),
...
# finally, scPCA
scpca_out <- scPCA(t(log1p(assay(sce_sub))),
                   t(log1p(assay(sce_background))),
                   n_centers = 4,
                   n_eigen = 2,
                   contrasts = contrasts,
...
reducedDims(sce_sub) <- SimpleList(PCA = pca_out$x[, 1:2],
                                   cPCA = cpca_out$x,
                                   scPCA = scpca_out$x)
sce_sub
```
...
for parallel evaluation as described in the `BiocParallel` documentation.
