...
framework [@Zappia2017]. This method simulates a scRNA-seq count matrix by way
of a gamma-Poisson hierarchical model, where the mean expression level of gene
$g_i,\; i = 1, ..., p$ is sampled from a gamma distribution, and the count
$x_{i, j}, \; j = 1, ..., n$ of cell $c_j$ is sampled from a Poisson
...
below, though without such a computationaly convenient data set.
...
`reducedDims` accessor. The results are presented in the following figure.
