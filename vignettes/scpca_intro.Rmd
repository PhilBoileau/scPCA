...
simultaneously. This `scPCA` package implements a combination of these methods,
dubbed sparse constrastive PCA (scPCA), that can accomplish this task. cPCA,
previously unavailable to `R` users, is also implmented.
...
(`n_centers = 4`). In general, this hyperparameter can be inferred *a priori*
from sample annotation variables (e.g. treatment groups, biological groups,
etc.), and empirical evidence suggests that the results of the algorithm are
robust to different values of `n_centers` [@boileau2020].
...
The hyperparemeters reponsible for contrastiveness and sparsity of the cPCA and
scPCA algorithms provided in this package are selected through a 
clustering-based hyperparemeter tuning framework (detailed in [@boileau2020]).
...
10-fold cross-valdidation when using this framework, though the number of folds
can vary with dataset size. Smaller samples will require fewer folds.
